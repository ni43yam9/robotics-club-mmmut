// Ported verbatim from pmndrs/examples "space-game" (src/3d/Rig.tsx).
...
let offset = 0;
export default function Rig({ children }: { children?: React.ReactNode }) {
  const group = useRef<THREE.Group>(null!);
  const rig = useRef<THREE.Group>(null!);
  const mutation = useStore((state) => state.mutation);
  const { fov, scale, binormal, normal, track, mouse } = mutation;
  // R3F types the default camera as ortho-or-perspective; App creates a perspective one
